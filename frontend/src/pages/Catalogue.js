    ...
    <div className="min-h-screen bg-[#0B141A] pb-32">
      ...
      <header className="sticky top-0 z-40 backdrop-blur-lg bg-[#0B141A]/80 border-b border-[#2A3942]">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-[#E9EDEF] font-['Outfit']" data-testid="catalogue-title">
              Catalogue
            </h1>
          </div>
          <button 
          ...
          >
            ...
            {getTotalItems() > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#25D366] text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold" data-testid="cart-count-badge">
                {getTotalItems()}
              </span>
            )}
          </button>
      ...
      <div className="relative h-48 md:h-64 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#0B141A] to-transparent flex items-end">
          <div className="p-6">
            <h2 className="text-3xl md:text-4xl font-bold text-white font-['Outfit']" data-testid="hero-title">
              Mr. Mutton
            </h2>
            <p className="text-[#E9EDEF] text-lg mt-2">Slow Cooked. Soul Satisfied.</p>
          </div>
        </div>
      </div>
        ...
        {Object.entries(groupedProducts).map(([category, categoryProducts]) => (
          <div key={category} className="mb-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl md:text-3xl font-semibold text-[#E9EDEF] font-['Outfit']" data-testid={`category-${category.toLowerCase().replace(/\s+/g, '-')}`}>
                {category}
              </h2>
              <button className="text-[#25D366] hover:text-[#1DA851] text-sm font-medium">See all</button>
            </div>
                  ...
                  >
                    <div className="flex md:flex-col gap-4 p-4 h-full">
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div>
                          <h3 className="text-base md:text-lg font-medium text-[#E9EDEF] mb-1" data-testid={`product-name-${product.id}`}>
                            {product.name}
                          </h3>
                          <p className="text-xs md:text-sm text-[#8696A0] mb-2">{product.description}</p>
                          <div className="flex items-center gap-2 mb-3">
                            <span className="text-lg md:text-xl font-bold text-[#E9EDEF]" data-testid={`product-price-${product.id}`}>
                        ...
                        <div className="flex items-center justify-end md:justify-start">
                          {quantity === 0 ? (
                            ...
                            <div className="flex items-center gap-2 bg-[#111B21] rounded-full px-2 py-1 border border-[#25D366]" data-testid={`quantity-controls-${product.id}`}>
                              <button
                              ...
                              <span className="text-[#E9EDEF] font-bold min-w-[24px] text-center" data-testid={`quantity-display-${product.id}`}>
                                {quantity}
                              </span>
                              <button
                                onClick={() => handleAddToCart(product)}
      ...
      {getTotalItems() > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-[#202C33] border-t border-[#2A3942] p-4 z-50 backdrop-blur-lg md:left-auto md:right-4 md:bottom-4 md:w-auto md:rounded-full md:border" data-testid="view-cart-footer">
          <button
            onClick={() => navigate('/cart')}
